...
export default function ModuleCompletionTab({ adminToken, adminApiBase, onRefreshCandidates }: Props) {
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ 
        phases: 'module,documents,offer,signing,joining',
        limit: String(limit), 
      ...
      console.error('Failed to load candidates for Module Completion:', e);
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmBypass = async () => {
    ...
    try {
      const res = await withAuth(`/api/admin/candidates/${candidateId}/bypass-module`, {
        method: 'POST',
      });
      if (res.ok) {
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <h2 style={{ fontSize: 16, fontWeight: 700, margin: 0, color: '#111827' }}>
        Question Collection module Completion Status
      </h2>
      ...
      {loading ? (
        ...
        <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', background: '#f9fafb', borderRadius: '8px' }}>
          No candidates in the Question Collection module phase.
        </div>
      ) : (
        <>
        <div style={{ background: 'white', borderRadius: '8px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            ...
            <tbody>
              {candidates.map((c) => {
                const status = c.moduleStatus || (c.moduleCompleted ? 'completed' : 'not_started');
                  ...
                  <tr key={c.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '16px', textAlign: 'right' }}>
                      {status !== 'completed' ? (
                        ...
                        >
                          {actionInProgress === c.id ? 'Bypassing...' : 'Bypass Module'}
                        </button>
                      ) : (
      ...
      {/* â”€â”€ Custom Confirmation Modal â”€â”€ */}
        ...
        >
          ...
          >
            ...
            <p style={{ margin: '0 0 24px 0', fontSize: '14px', color: '#4b5563', lineHeight: '1.5' }}>
              Are you sure you want to bypass the Question Collection module requirement for <strong>{confirmModal.candidateName}</strong>? 
              This will mark the module step as completed and transition the candidate to the document upload phase.
            </p>
